...
export function resolveScreenshot(path: string | undefined, locale: string): string {
  ...
  if (path.includes("{locale}")) {
    resolved = path.replace(/\{locale\}/g, locale);
  }
  if (resolved.endsWith("/01.png")) {
    resolved = resolved.replace("/01.png", "/01.jpg");
  }
  if (resolved.endsWith("/02.png")) {
    resolved = resolved.replace("/02.png", "/02.jpg");
  }
  return resolved;
}
